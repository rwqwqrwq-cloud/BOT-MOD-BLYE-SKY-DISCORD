...
const loadCommands = () => {
    // Load from commands/ folder (individual files)
    ...
    // Load from root-level module files (moderation.js, utility.js, etc.)
    const moduleFiles = fs.readdirSync(__dirname)
        .filter(f => f.endsWith('.js') && !['bot.js'].includes(f));
    for (const file of moduleFiles) {
        try {
            const mod = require(path.join(__dirname, file));
            // Modules export named commands as an object of { name, execute, ... }
            for (const [key, command] of Object.entries(mod)) {
